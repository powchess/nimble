...
describe('ripemd160', () => {
	test('empty', () => {
		const expected = Array.from(bsv.crypto.Hash.ripemd160(bsv.deps.Buffer.from([])));
		const actual = Array.from(ripemd160(new Uint8Array([])));
	...
	test('performance', () => {
		...
		for (let i = 0; i < 1000; i++) {
			ripemd160(new Uint8Array([1, 2, 3, 4, 5]));
		}
